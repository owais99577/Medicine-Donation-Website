...
def addrec3():
   if request.method == 'POST':
       try:
           ...
           with sqlite3.connect("newdb.db") as conn:
               cur = conn.cursor()
               cur.execute(''' INSERT INTO user_login (email,password) VALUES (?,?)''',(emailid,password)) 
               print('insert executed')
               conn.commit()
...
def addrec4():
   if request.method == 'POST':
       try:
         ...
         donornumber = request.form['no']
         donoraddress = request.form['addr']
         donatingmedicine = request.form['medname']
         donatinggenericname = request.form['Gename']
         donatingquantity = request.form['quantity']
         recieversmail  = request.form['recmail']
         
         with sqlite3.connect("newdb.db") as conn:
            cur = conn.cursor()
         
            
            cur.execute('''INSERT INTO userdon (name,email,num, addr, med, gen, quantity, reciever) 
